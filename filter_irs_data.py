# Create matrix representation of weighted directed graph
# following http://cpsc.ualr.edu/srini/DM/chapters/review5.3.html
# but where each cell of the matrix has one row in the output file

# Traverse the input file line-by-line, determine if the statistic describes county-to-
# county migration, and write out if so.
...
with open('irs/countyoutflow1213.csv') as csvfile:
    ...
    for row in reader:
        ...
        # restrict to county-to-county information, not aggregates
        if int(fromStateFIPSid) <= 56 and int(toStateFIPSid) <= 56 \
            and int(fromCountyFIPSid) > 0 and int(toCountyFIPSid) > 0:
